...
```bash
echo "[$(date '+%Y-%m-%d %H:%M:%S')] [INFO] [stage-05] message" >> {RUN_DIR}/pipeline/execution.log
```

Log this milestone:
1. After calibration: `Calibration complete | n_grades={N} | target_ct={X}% | achieved_ct={X}% | pd_ordering={valid/invalid}`
